...
from src.adjustment.adjustment_sets_utils import AdjustmentSetUtils as au
...
class FrontdoorAdjustment():
    ...
    def FindFDSet(G, X, Y, I=[], R=[]):
        ...
        try:
            ...
            CPG = ProjectionUtils.unproject(CPG)

            if au.TestSep(CPG, X, Y, Z) == True:
                return Z
            else:
                return None
    ...
    def GetCand2ndFDC(G, X, I=[], R=[]):
        ...
        GXbar = ProjectionUtils.unproject(GXbar)

        for v in R:
            if au.TestSep(GXbar, X, [v], []) == False:
    ...
    def ListFDSets(G, X, Y, I=[], R=[], limit=1e8):
        ...
        FrontdoorAdjustment.__ListFDSets(G, X, Y, I, R, admissibleSets, limit)
