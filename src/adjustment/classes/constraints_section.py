...
from src.adjustment.classes.constraints_task import constraintsIDefName, constraintsRDefName, constraintsSDefName, constraintsIDef, constraintsRDef, constraintsSDef
...
errors = {
    ...
    'Rfirst': 'Please specify \'R\' constraints before \'S\'.',
    'IinR': '\'I\' must be a subset or equal to \'R\'.',
    'RandS': '\'R\' and \'S\' must be disjoint.',
...
class ConstraintsSection(Section):
    ...
    def parse(self, lines, parsedData={}):
        ...
        try:
            for line in lines:
                ...
                elif constraints['label'] == constraintsSDefName:
                    task.sets[constraintsSDefName] = constraints['names']

                    taskDef = None

                    for df in task.definitions:
                        if df.name == constraintsSDefName:
                            taskDef = df
                            break

                    if taskDef is None:
                        task.definitions.append(constraintsSDef)
    ...
    def constraintsFromString(self, line, graph, task):
        ...
        if paramName != constraintsIDefName and paramName != constraintsRDefName and paramName != constraintsSDefName:
        ...
        # S and R must be disjoint
        if paramName == constraintsSDefName:
            if constraintsIDefName not in task.sets:
                raise ParsingError(errors['Rfirst'])

            Rnames = task.sets[constraintsRDefName]
            Rnodes = list(filter(
                lambda n: n['type_'] == basicNodeType.id_, gu.getNodesByName(Rnames, graph)))

            if len(su.intersection(Rnodes, nodes, 'name')) > 0:
                raise ParsingError(errors['RandS'])
