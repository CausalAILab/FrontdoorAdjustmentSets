...
class ProjectionUtils():
    ...
    def projectOver(G, V):
        ...
        latentsInOrder = list(
            filter(lambda n: n['type_'] == latentNodeType.id_, orderedV))
        ...
        for latent in latentsInOrder:
            ...
            for parent in parents:
                for child in children:
                    dirPtoC = gu.getEdgeByName(
                        parent['name'], child['name'], graph, directedEdgeType)

                    # add latent nodes/edges info to new edge metadata
                    dirPtoL = gu.getEdgeByName(
                        parent['name'], latent['name'], graph, directedEdgeType)
                    dirLtoC = gu.getEdgeByName(
                        latent['name'], child['name'], graph, directedEdgeType)

                    dirPtoLinG = gu.getEdgeByName(
                        parent['name'], latent['name'], G, directedEdgeType)
                    dirLtoCinG = gu.getEdgeByName(
                        latent['name'], child['name'], G, directedEdgeType)
            ...
            # step 4
            incidentBidirected = list(filter(
                lambda e: e['type_'] == bidirectedEdgeType.id_, gu.getIncoming(latent, graph)))
            ...
            incidentNodes = su.union(
                children, incidentNodesViaBidirected, 'name')
            ...
            for pair in pairs:
                ...
                dirLtoX = gu.getEdgeByName(
                    latent['name'], X['name'], graph, directedEdgeType)
                dirLtoY = gu.getEdgeByName(
                    latent['name'], Y['name'], graph, directedEdgeType)
                biLX = gu.getEdgeByName(
                    latent['name'], X['name'], graph, bidirectedEdgeType)
                biLY = gu.getEdgeByName(
                    latent['name'], Y['name'], graph, bidirectedEdgeType)

                if not biLX:
                    biLX = gu.getEdgeByName(
                        X['name'], latent['name'], graph, bidirectedEdgeType)

                if not biLY:
                    biLY = gu.getEdgeByName(
                        Y['name'], latent['name'], graph, bidirectedEdgeType)
                ...
                if case1 or case2 or case3:
                    biXY = gu.getEdgeByName(
                        X['name'], Y['name'], graph, bidirectedEdgeType)
                    biYX = gu.getEdgeByName(
                        Y['name'], X['name'], graph, bidirectedEdgeType)
    ...
    def unproject(G):
        ...
        bidirectedEdges = list(
            filter(lambda e: e['type_'] == bidirectedEdgeType.id_, graph.edges))
